...
uint32_t volatile *const pPortAInputReg=(uint32_t*)0x40020010;
//Enable clk for GPIOA and GPIOD
*pclkreg|=(1<<0);
*pclkreg|=(1<<3);
//configuring pd12 as output mode
*pPortDModeReg&=~(3<<24);
*pPortDModeReg|=(1<<24);
//configuring pA0 as input mode
*pPortAModeReg&=~(3<<0);
//Read the data
while(1){
uint8_t pin_read=(uint8_t)(*pPortAInputReg&0x1);
if(pin_read){
	*pPortDOutReg|=(1<<12);
}else{
	*pPortDOutReg&=~(1<<12);
